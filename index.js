...
console.log(makeSmartPerson("Maddy"));
...
function getLastCarInfo(inventory) {
  const make = inventory[inventory.length-1].car_make;
  const model = inventory[inventory.length-1].car_model;
...
function getGermanCars(inventory) {
  ...
  for (let i = 0; i < inventory.length; i++) {
    if (makes.indexOf(inventory[i].car_make) > -1) {
      arr.push(inventory[i]);
    }
  }
